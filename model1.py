...
class XGBOOST:
    ...
    def predict_classes(self):
        ...
        classes = self.model.predict(self.test[predictors])
        return classes 
    
    def evaluate(self):
        classes = self.predict_classes()
        actual = self.test['Target']  
        acc = classification_report(actual, classes)
        return acc 

class SaveModel(XGBOOST):
    def save_model(self, model_path='stock-high-low3.pkl'):
        joblib.dump(self.model, open(model_path, 'wb'))
...
pipeline_xgboost2 = SaveModel(df)
pipeline_xgboost2.save_model()
